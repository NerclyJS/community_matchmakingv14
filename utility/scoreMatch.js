...
async function scoreMatch(client, interaction, matchId, winner) {
        ...
        if (lastUsed && now - lastUsed < 40000) {
            ...
            return interaction.reply({
                ...
                embeds: [new EmbedBuilder()
                    .setDescription(`❗ Actions for this match are still ongoing`)
                    .setColor('#ff0557')
                        .setAuthor({
      ...
      qdb.set(`scoreCooldown_${matchId}`, now);
        ...
        try {
            ...
            const voidedMatchesCollection = db.collection('voidedMatches');
...
return;
            ...
            const voidedMatch = await voidedMatchesCollection.findOne({ match_id: matchId });
    ...
    interaction.editReply({
        ...
        embeds: [new EmbedBuilder()
            .setTitle('Error')
            .setDescription('This match was voided.')
            .setColor('#ff0557')
                .setAuthor({
        name: 'Critical Ops Esports', 
        iconURL: client.user.displayAvatarURL()}) ],
...
return;
            ...
            if (winner === "void") {
                const errorEmbed = new EmbedBuilder()
                    .setTitle('Voided')
                    .setDescription('This match has been voided.')
                    .setColor('#ff0557')
                      .setAuthor({
        name: 'Critical Ops Esports', 
        iconURL: client.user.displayAvatarURL()}) 
            
                interaction.editReply({content: `<@${interaction.user.id}>`, embeds: [errorEmbed] }).catch(() => {
                    interaction.channel.send({ content: `<@${interaction.user.id}>`, embeds: [errorEmbed] }).catch(console.error);
                });
            
            
                await voidedMatchesCollection.insertOne(match);
...
return
 ...
 if (match.isDouble) {
    interaction.editReply({  content: `<@${interaction.user.id}>`, embeds: [resultEmbed, doubleEmbed], components: [] }).catch(() => {
        interaction.channel.send({ content: `<@${interaction.user.id}>`, embeds: [resultEmbed, doubleEmbed], components: [] }).catch(console.error);
    });
} else {
    interaction.editReply({  content: `<@${interaction.user.id}>`,embeds: [resultEmbed], components: [] }).catch(() => {
